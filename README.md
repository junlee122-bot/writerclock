...
- 시각 직접 탐색(1분·10분 단위), 즐겨찾기와 공유, 같은 시각에 문장이 여러 개일 때 다른 문장 보기
- 큰 시각과 초, 종이색·먹색 바탕의 세로/가로 전시형 레이아웃
- 인용문 명조(Noto Serif KR)와 시계 숫자 폰트를 앱에 포함해 모든 기기에서 같은 조판
- 접어서 여는 시간 탐색과 눌러서 확인하는 문장 출전
- 자동/밝은/어두운 테마, 글자 크기, 원터치 거치 모드, 화면 켜짐 유지
- 거치 모드의 몰입형 전체화면, 야간 감광, 번인 방지 미세 이동, 현재 시각 자동 복귀와 Esc 종료
